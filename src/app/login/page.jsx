...
import { Mail, Lock, Activity, LogIn } from "lucide-react";
...
const LoginPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#050816] px-5 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute bottom-[-10%] left-[-10%] w-80 h-80 bg-[#21B7E2]/10 blur-[120px] rounded-full"></div>
        ...
        <form className="space-y-5" onSubmit={onSubmit}>
          <div className="relative group">
            ...
            <input type="email" name="email" placeholder="Email Address" required
              className="w-full bg-[#050816] border border-gray-800 rounded-2xl py-4 pl-12 pr-4 text-sm text-white outline-none focus:ring-2 focus:ring-[#21B7E2]/50 transition-all placeholder:text-gray-600" />
          </div>
          ...
          <div className="relative group">
            ...
            <input type={showPassword ? "text" : "password"} name="password" placeholder="Password" required
              className="w-full bg-[#050816] border border-gray-800 rounded-2xl py-4 pl-12 pr-12 text-sm text-white outline-none focus:ring-2 focus:ring-[#21B7E2]/50 transition-all placeholder:text-gray-600" />
          ...
          <button disabled={loading} className="w-full py-4 bg-[#21B7E2] text-[#050816] font-black rounded-2xl shadow-xl shadow-cyan-500/20 hover:bg-white transition-all text-lg active:scale-95">
            {loading ? "Signing in..." : "Login Now"}
          </button>

          <div className="flex items-center my-6">
            <div className="flex-1 h-px bg-gray-800"></div>
            <span className="px-4 text-[10px] font-black text-gray-600 uppercase tracking-[0.3em]">OR</span>
            <div className="flex-1 h-px bg-gray-800"></div>
          </div>
        </form>

        <button onClick={googleLogin} className="w-full py-4 bg-[#050816] border border-gray-800 text-white rounded-2xl hover:bg-white hover:text-[#050816] transition-all font-bold flex items-center justify-center gap-3 mb-8">
          <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.66l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>
          Google
        </button>

        <p className="text-center text-gray-500 text-sm">
          New to DocAppoint?
          <Link href="/register" className="text-[#21B7E2] hover:underline font-bold ml-2">Sign Up</Link>
        </p>
      </div>
    </div>
  );
