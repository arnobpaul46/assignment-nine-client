...
const TopDoctors = () => {
        ...
        const fetchDoctorsFromDB = async () => {
            try {
                ...
                specialties.forEach(spec => {
                    const doctorsInSpec = allDoctors.filter(d => d.specialty === spec);
                    if (doctorsInSpec.length > 0) {
                        const topInSpec = doctorsInSpec.reduce((prev, current) => 
    ...
    if (loading) return <div className="bg-[#050816] py-10 text-center text-[#21B7E2]">Loading Top Doctors...</div>;
