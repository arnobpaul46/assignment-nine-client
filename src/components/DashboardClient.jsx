...
import { User, Mail, Edit3, Calendar, BadgeCheck, Trash2, Edit } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const confirmDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('access-token');
            const backendUri = process.env.NEXT_PUBLIC_BACKEND_URI;
            const res = await fetch(`${backendUri}/bookings/${deleteId}`, { 
                method: 'DELETE',
                headers: {
                    'authorization': `Bearer ${token}`
                }
            ...
            if (res.ok) {
                toast.success("Appointment Cancelled Successfully!", { theme: "dark" });
                setDeleteId(null);
                fetchAppointments();
            }
        } catch (error) {
        ...
        <main className="min-h-screen bg-[#050816] text-white py-10">
            <div className="max-w-[90%] lg:max-w-[80%] mx-auto">
                <div className="flex flex-col lg:grid lg:grid-cols-4 gap-10">
                    <div className="lg:col-span-1 space-y-3">
                        <button onClick={() => setActiveTab('appointments')} className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl font-bold transition-all ${activeTab === 'appointments' ? 'bg-[#21B7E2] text-[#050816]' : 'bg-white/5 text-gray-500 hover:text-white'}`}><Calendar size={20} /> My Appointments</button>
                        ...
                        {activeTab === 'appointments' ? (
                                ...
                                {loading ? <p className="text-gray-500">Fetching appointments...</p> :
                                    appointments.length > 0 ? (
                                        appointments.map(app => (
                                            <div key={app._id} className="bg-[#0a0f20] border border-gray-800 p-6 rounded-[2.5rem] flex flex-col md:flex-row justify-between items-start md:items-center gap-6 group hover:border-[#21B7E2]/40 transition-all shadow-lg">
                            ...
                            <div className="bg-[#0a0f20] border border-gray-800 rounded-[3rem] p-8 md:p-12 shadow-2xl">
                                <div className="flex flex-col md:flex-row items-center gap-8 mb-10">
                                    <div className="w-28 h-28 rounded-[2rem] border-2 border-[#21B7E2] overflow-hidden"><img src={session?.user?.image || "https://api.dicebear.com/7.x/avataaars/svg"} className="w-full h-full object-cover" alt="Profile" /></div>
                                    <div className="text-center md:text-left">
                                        ...
                                        <div className="mt-3 bg-green-500/10 text-green-500 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest w-fit inline-flex items-center gap-2"><BadgeCheck size={14} /> Verified Account</div>
                                    </div>
                                </div>
                                {!isEditing ? (
                                    ...
                                    <form onSubmit={handleUpdateProfile} className="space-y-5 pt-6 border-t border-gray-800 animate-in fade-in duration-500">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                            <input type="text" value={newName} required className="bg-[#050816] border border-gray-800 rounded-xl p-4 outline-none focus:ring-2 focus:ring-[#21B7E2] text-white" onChange={(e) => setNewName(e.target.value)} />
                                            <input type="url" value={newImage} placeholder="Image URL" className="bg-[#050816] border border-gray-800 rounded-xl p-4 outline-none focus:ring-2 focus:ring-[#21B7E2] text-white" onChange={(e) => setNewImage(e.target.value)} />
                                        </div>
                                        <div className="flex gap-3">
            ...
            {deleteId && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="bg-[#0a0f20] border border-red-500/20 p-8 rounded-[2.5rem] max-w-sm w-full text-center shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <div className="flex gap-3">
                            <button onClick={confirmDelete} className="flex-1 bg-red-500 text-white py-3 rounded-xl font-bold hover:bg-red-600 transition-all">Yes, Cancel</button>
                            <button onClick={() => setDeleteId(null)} className="flex-1 bg-gray-800 py-3 rounded-xl font-bold hover:bg-gray-700">No</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {updateData && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                    <div className="bg-[#0a0f20] border border-[#21B7E2]/20 p-8 rounded-[2.5rem] max-w-sm w-full shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <form onSubmit={handleUpdateAppointment} className="space-y-4">
                            <div>
                                ...
                                <input type="date" value={updateData?.appointmentDate || ""} onChange={(e) => setUpdateData({ ...updateData, appointmentDate: e.target.value })} className="w-full bg-[#050816] border border-gray-800 p-4 rounded-2xl mt-1 text-white outline-none focus:ring-2 focus:ring-[#21B7E2]" />
                            </div>
                            <div>
                                <label className="text-[10px] text-gray-500 font-bold uppercase ml-1">New Time</label>
                                <input type="text" value={updateData?.appointmentTime || ""} onChange={(e) => setUpdateData({ ...updateData, appointmentTime: e.target.value })} className="w-full bg-[#050816] border border-gray-800 p-4 rounded-2xl mt-1 text-white outline-none focus:ring-2 focus:ring-[#21B7E2]" />
                            </div>
                            <div className="flex gap-3 mt-6">
                                <button type="submit" className="flex-1 bg-[#21B7E2] text-[#050816] py-3 rounded-xl font-bold hover:bg-white transition-all">Save</button>
                                <button type="button" onClick={() => setUpdateData(null)} className="flex-1 bg-gray-800 py-3 rounded-xl font-bold hover:bg-gray-700">Back</button>
                            </div>
                        </form>
                    </div>
                </div>
